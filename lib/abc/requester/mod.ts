...
export interface ContractFlowOptions {
  ...
  /**
   * Pluggable user_data composition (see @publicdomainrelay/cloud-init-common
   * buildUserData). A caller-supplied base cloud-config is patched with the
   * selected transport module plus any extra modules, so callers can add e.g.
   * wootty-token-handoff style code without rebuilding the whole cloud-init.
   */
  userData?: {
    ...
    /** Overrides winning over base + modules (e.g. scalar toggles). */
    overrides?: Partial<import("@publicdomainrelay/cloud-init-common").UserDataPatch>;
  };
  /**
   * @deprecated Use `userData.base` instead. When set and `userData.base` is
   * unset, the default tunnel provisioning is patched into this base.
   */
  baseUserData?: string;
  /**
   * Guest capabilities spanning the contract. Each contributes cloud-init before
   * the RFP is sent, receives its grant once a bid wins, loses it when the VM
   * delete event is sent, and is disposed at the end. The flow never names a
   * capability, so adding one does not change this module.
   */
  capabilities?: import("@publicdomainrelay/guest-capability-abc").GuestCapability[];
  /**
   * When set, fully replaces the composed cloud-init with the value this
   * returns. Wins over userData. The argument is the ssh authorized key
   * generated by the sshProvider; the caller closes over any additional
   * context. Returns a #cloud-config YAML string.
   */
